...
/// @dev Implements ISparsityApp to integrate with the Sparsity platform
contract BTCPriceOracle is ISparsityApp {
    ...
    modifier onlyOracle() {
        if (msg.sender != oracle && !isTEEWallet[msg.sender])
            revert OnlyOracle();
        _;
    }
    ...
    /// @dev Implements ISparsityApp interface - can only be called by registry
